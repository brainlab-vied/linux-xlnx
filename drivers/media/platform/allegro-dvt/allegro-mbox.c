...
{
	struct allegro_dev *dev = mbox->dev;
	int dec = (dev->devtype->inst_type == ALLEGRO_INST_DECODER) ? 1 : 0; // special change in 2022 allegre
	...
	err = allegro_decode_mail(msg, tmp, dec); // special change in 2022 allegre
	if (err)
		goto out;
